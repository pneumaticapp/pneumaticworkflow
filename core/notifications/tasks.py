...
from pneumatic_backend.processes.models import TaskForList
...
def _send_new_task_notification(
    ...
    for (user_id, user_email, is_subscriber) in recipients:
        if is_subscriber:
            _send_notification(
                logging=logging,
                account_id=account_id,
...
def send_new_task_notification(**kwargs):
    ...
    _send_new_task_websocket(
        task_id=kwargs.get('task_id'),
        recipients=[(user_id, email) for user_id, email, *_ in recipients],
