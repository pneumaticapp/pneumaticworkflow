...
def test_continue_task__current_task_not_started__ok(mocker):
    ...
    call_args = send_new_task_notification_mock.call_args
    expected_recipients = [
        (guest.id, guest.email, guest.is_new_tasks_subscriber),
        (user.id, user.email, user.is_new_tasks_subscriber),
        (
            unsubscribed_performer.id,
            unsubscribed_performer.email,
            unsubscribed_performer.is_new_tasks_subscriber
        )
    ]
    actual_recipients = call_args[1]['recipients']
    assert set(actual_recipients) == set(expected_recipients)
    assert call_args[1]['logging'] == user.account.log_api_requests
    assert call_args[1]['account_id'] == user.account_id
    assert call_args[1]['task_id'] == task.id
    assert call_args[1]['task_name'] == task.name
    assert call_args[1]['task_description'] == task.description
    assert call_args[1]['workflow_name'] == workflow.name
    assert call_args[1]['template_name'] == template.name
    assert call_args[1]['workflow_starter_name'] == workflow_starter.name
    assert call_args[1]['workflow_starter_photo'] == workflow_starter.photo
    assert call_args[1]['due_date_timestamp'] == task.due_date.timestamp()
    assert call_args[1]['logo_lg'] == logo_lg
    assert call_args[1]['is_returned'] is False
...
def test_continue_task__current_started__ok(mocker):
    ...
    assert call_args[1]['task_description'] == task.description
    assert call_args[1]['workflow_name'] == workflow.name
    assert call_args[1]['template_name'] == workflow.template.name
    assert call_args[1]['workflow_starter_name'] == user.name
    assert call_args[1]['workflow_starter_photo'] is None
    assert call_args[1]['due_date_timestamp'] is None
    assert call_args[1]['logo_lg'] is None
    assert call_args[1]['is_returned'] is False
    delete_task_guest_cache_mock.has_calls([
        mocker.call(task_id=prev_task.id),
        mocker.call(task_id=task.id)
...
def test_continue_task__current_started_task_1_with_performer_and_guest__ok(
    ...
    expected_recipients = [
        ...
        (
            one_performer.id,
            one_performer.email,
            one_performer.is_new_tasks_subscriber
        )
    ]
    actual_recipients = call_args[1]['recipients']
    assert set(actual_recipients) == set(expected_recipients)
...
def test_continue_task__returned_task_1_with_performer_group__ok(
    mocker
):
    ...
    group = create_test_group(user.account, users=[one_performer, user])
    workflow.save(update_fields=['current_task', 'is_urgent'])
    task = workflow.tasks.get(number=1)
    ...
    assert call_args[1]['due_date_timestamp'] is None
    assert call_args[1]['logo_lg'] is None
    assert call_args[1]['is_returned'] is True


def test_continue_task__performer_group_directly_status_delete__ok(
    mocker
):
    ...
    group = create_test_group(user.account, users=[one_performer, user])
    group2 = create_test_group(user.account, users=[one_performer, user])
    workflow.save(update_fields=['current_task', 'is_urgent'])
    task = workflow.tasks.get(number=1)
    TaskPerformer.objects.filter(
        task_id=task.id
...
def test_continue_task__workflow_starter_not_performer__ok(
    mocker
):
    ...
    group = create_test_group(user.account, users=[one_performer])
    workflow.save(update_fields=['current_task', 'is_urgent'])
    task = workflow.tasks.get(number=1)
    TaskPerformer.objects.filter(
        task_id=task.id
...
def test_complete_task__performer_complete_current_task__ok(mocker):
    ...
    send_removed_task_notification_mock.assert_called_once_with(
        task_id=task.id,
        recipients=[
            (account_owner.id, account_owner.email),
            (user.id, user.email)
        ],
        account_id=task.account_id
    )
...
def test_complete_task__user_with_deleted_performer_on_other_task__ok(mocker):
    """
    Test verifies that a user receives task completion notification even
    if they have a TaskPerformer with directly_status=DELETED on another task.
    """
    ...
    workflow = create_test_workflow(user=user, tasks_count=2)
    task = workflow.tasks.get(number=1)
    any_workflow = create_test_workflow(account_owner, tasks_count=1)
    any_task = any_workflow.current_task_instance
    TaskPerformer.objects.create(
        task=any_task,
        user=user,
        directly_status=DirectlyStatus.DELETED
...
def test_complete_task__unsubscribed_performer__not_sent(mocker):
    ...
    assert set(call_args['recipients']) == expected_user_ids
    assert call_args['task_id'] == task.id
    assert call_args['account_id'] == task.account_id
    complete_task_webhook_mock.assert_not_called()
    end_workflow_mock.assert_not_called()
    next_task = workflow.tasks.get(number=2)
    action_method_mock.assert_called_once_with(next_task)
