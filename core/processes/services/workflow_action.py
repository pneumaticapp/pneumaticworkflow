...
class WorkflowActionService:
    ...
    def continue_task(self, task: Task, is_returned: bool = False):
        ...
        if (
            ...
            user_ids = [user[0] for user in recipients]
            if (
                self.workflow.current_task == 1 and
                not is_returned and
                self.workflow.workflow_starter and
                self.workflow.workflow_starter_id in user_ids
            ):
                recipients = [
                    user for user in recipients
                    if user[0] != self.workflow.workflow_starter_id
                ]
                send_new_task_websocket.delay(
                    task_id=task.id,
                    recipients=[(
                        self.workflow.workflow_starter.id,
                        self.workflow.workflow_starter.email
                    )],
                    account_id=task.account_id
                )
            ...
            # Websocket, Email and Push notification
    ...
    def _return_workflow_to_task(
        ...
        if workflow_is_running:
            ...
            send_removed_task_notification.delay(
                ...
                recipients=recipients,
                account_id=revert_from_task.account_id,
            )
