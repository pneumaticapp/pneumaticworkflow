...
def test_retrieve__sub_workflows__ok(api_client, mocker):
    ...
    current_task = data['tasks'][0]
    assert current_task['id'] == task_1.id
    assert current_task['name'] == task_1.name
    assert current_task['number'] == 1
    assert current_task['due_date_tsp'] == task_1.due_date.timestamp()
