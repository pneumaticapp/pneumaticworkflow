...
import { getTaskList, getTasksSettings, getTotalTasksCount } from '../../selectors/tasks';
...
jest.mock('../../../utils/history', () => ({
  ...
  history: { push: jest.fn(), replace: jest.fn(), location: { pathname: '/tasks' } },
}));
...
describe('handleRemoveTask', () => {
  ...
  it('selects currentTask and removes from list on tasks route', () => {
    ...
    const removeStep = gen.next({ id: 99 } as never);
    expect(removeStep.done).toBe(false);

    const refreshStep = gen.next();
    expect(refreshStep.done).toBe(false);
...
describe('handleAddTask', () => {
  ...
  it('removes reactivated task from Completed list', () => {
    ...
    expect(gen.next().value).toEqual(select(getTaskList));
    expect(gen.next({ items: [] } as never).value).toEqual(select(getTotalTasksCount));
  ...
  it('inserts new task on Active list', () => {
    ...
    ).toEqual(call(handleInsertNewTask, insertNewTask(task)));
