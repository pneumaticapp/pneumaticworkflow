...
export function useWorkflowsTableData({
  ...
  const maxPerformersCount = useMemo(
    () => workflowsList.items.reduce((maximum, workflow) => Math.max(maximum, workflow.selectedUsers?.length ?? 0), 0),
    [workflowsList.items],
  );
  const { savedGlobalWidths, savedOptionalWidths, performerColumnMinWidth, performerColumnWidth } =
  ...
  const data = useMemo<TableColumns[]>(
    () =>
      workflowsList.items.map((workflow) => {
        const row = {
