...
import { getAttachmentTypeByUrl, getAttachmentTypeByFilename } from '../../Attachments/utils/getAttachmentType';
...
function hasUrl(att: { url?: string } | undefined): att is NonNullable<typeof att> & { url: string } {
...
async function uploadFilesAndInsert(
  ...
  try {
    ...
    uploaded.filter(hasUrl).forEach((att) => {
      const type = getAttachmentTypeByFilename(att.name) ?? getAttachmentTypeByUrl(att.url) ?? 'file';
