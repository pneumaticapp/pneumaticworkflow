...
export function createInlineStyleButton({ style, tooltipText, children }: ICreateInlineStyleButtonProp) {
  return function InlineStyleButton(props: React.PropsWithChildren<IExtendedToolbarChildrenProps>) {
    ...
    const toggleStyle = (event: MouseEvent): void => {
      ...
      const newEditorState = RichUtils.toggleInlineStyle(EditorState.forceSelection(editorState, newSelection), style);
    ...
    const { theme } = props;
    const className = styleIsActive() ? classnames(theme.button, theme.active) : theme.button;
    ...
    return (
      <div className={theme.buttonWrapper} onMouseDown={preventBubblingUp}>
        ...
        <button ref={buttonRef} className={className} onClick={toggleStyle} type="button" children={children} />
      </div>
    );
  };
}
...
const getBlockEntitiesRanges = (contentState: ContentState, blockKey: string) => {
  ...
  const sortedEntitiesRanges = [...blockLinks, ...linksWithNames, ...blockMentions, ...blockVariables]
