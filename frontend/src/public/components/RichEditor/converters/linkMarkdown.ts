...
export const MARKDOWN_LINK: TextMatchTransformer = {
  ...
  export: (node, _exportChildren, exportFormat) => {
    ...
    const linkContent = title ? `[${escapedText}](${node.getURL()} "${title}")` : `[${escapedText}](${node.getURL()})`;
  ...
  getEndIndex: (textNode, match) =>
    getMarkdownLinkMatchEndIndex(textNode.getTextContent(), match.index ?? 0, parseGeneralMarkdownLinkFromStart),
  replace: (textNode, match) => {
    const parsedMatch = parseGeneralMarkdownLinkFromStart(textNode.getTextContent()) ?? match;
