...
export const DropdownArea = React.forwardRef<DropdownAreaHandle, IDropdownAreaProps>(
  ({ children, toggle, title, containerClassName, placement = 'bottom-start', onOpen, onClose }, ref) => {
    ...
    return (
      <OutsideClickHandler onOutsideClick={onOutsideClick}>
        <div className={classnames(styles['container'], isOpen && styles['container_is-open'], containerClassName)}>
          <button className={styles['toggle']} onClick={() => setIsOpen(!isOpen)} ref={setReferenceElement}>
            {renderToggle()}
          </button>
          <div className={styles['content']} ref={setPopperElement} style={popperStyles.popper} {...attributes.popper}>
            {children}
          </div>
        </div>
      </OutsideClickHandler>
