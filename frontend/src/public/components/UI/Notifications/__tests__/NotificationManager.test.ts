...
describe('NotificationManager', () => {
  describe('notifyApiError', () => {
    it('shows warning for 4xx errors', () => {
      ...
      NotificationManager.notifyApiError({ status: 400 }, { message: 'Bad request' });
    ...
    it('shows error for 5xx errors', () => {
      ...
      NotificationManager.notifyApiError({ status: 500 }, { message: 'Internal server error' });
    ...
    it('shows warning when status is undefined', () => {
      ...
      NotificationManager.notifyApiError({}, { message: 'Unknown error' });
    ...
    it('shows warning for File Service 413 (FILE_003)', () => {
      ...
      NotificationManager.notifyApiError({ status: 413 }, { message: 'file-service.size-exceeded' });
    ...
    it('shows error for File Service 503 (STORAGE_002)', () => {
      ...
      NotificationManager.notifyApiError({ status: 503 }, { message: 'file-service.upload-failed' });
    ...
    it('shows warning for File Service 401 (AUTH_001)', () => {
      ...
      NotificationManager.notifyApiError({ status: 401 }, { message: 'file-service.auth-failed' });
    ...
    it('shows warning for File Service 403 (PERM_001)', () => {
      ...
      NotificationManager.notifyApiError({ status: 403 }, { message: 'file-service.permission-denied' });
    ...
    it('shows warning for File Service 404 (FILE_001)', () => {
      ...
      NotificationManager.notifyApiError({ status: 404 }, { message: 'file-service.file-not-found' });
  ...
  describe('create() preserves notification type without override', () => {
    ...
    it('preserves type=warning for payment declined message (alertErrorMessages regression)', () => {
      ...
      const notifications = changeHandler.mock.calls[0][0];
      expect(notifications).toEqual(
        expect.arrayContaining([expect.objectContaining({ id: 'test-2', type: 'warning' })]),
      );
    });
  });
});
