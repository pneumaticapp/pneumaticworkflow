...
class FieldSetRuleSetService(BaseModelService):
    ...
    def _create_groups_or(self, instance_template: FieldSetTemplateRuleSet):
        for group_or_template in instance_template.groups_or.all():
            ...
            if and_objects:
                FieldSetRuleGroupAnd.objects.bulk_create(
                    and_objects,
                )
    ...
    def _get_error_message(
        ...
        if len(expected_values) == 1:
            op = (operators or [''])[0]
            if op == FieldSetRuleOperator.SUM_GREATER_THAN:
                return (
                    f'The sum of the fields must be '
                    f'greater than {expected_values[0]}.'
                )
            if op == FieldSetRuleOperator.SUM_LESS_THAN:
                return (
                    f'The sum of the fields must be '
                    f'less than {expected_values[0]}.'
                )
