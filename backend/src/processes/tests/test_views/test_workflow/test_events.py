...
def test_events__ordering_date_inverted__ok(api_client):
    ...
    WorkflowEventService.task_complete_event(task=task_1, user=user)
    task_2 = workflow.tasks.get(number=2)
    task_2.date_started = timezone.now()
    task_2.save(update_fields=['date_started'])
    WorkflowEventService.task_started_event(task_2)
...
def test_retrieve__external_workflow__ok(
    ...
    assert response_data['task']['id'] == task.id
...
def test_retrieve__task_started__ok(api_client):
    ...
    task_1.due_date = due_date
    task_1.save()
    event = WorkflowEventService.task_started_event(task_1)
    api_client.token_authenticate(user)
    ...
    assert task_data['id'] == task_1.id
    assert task_data['description'] == task_1.description
    assert task_data['name'] == task_1.name
    assert task_data['number'] == task_1.number
...
def test_retrieve__all_tasks_skipped__only_one_end_workflow_event(api_client):
...
def test_retrieve__two_tasks_delayed__get_two_delay_events(api_client):
    ...
    assert workflow.is_delayed
    task_1 = workflow.tasks.get(number=1)
    assert task_1.is_delayed
...
def test_retrieve__return_workflow_to_task__one_event(api_client):
    ...
    template_task_3.conditions.all().delete()

    api_client.token_authenticate(owner)

    response_create = api_client.post(f'/templates/{template.id}/run')
    # three workflow tasks active now
...
def test_events__task_complete_fieldsets_present__ok(api_client):
    ...
    """
    GET workflow events: TASK_COMPLETE row includes non-null task.fieldsets
    when the task has at least one FieldSet.
    """
    ...
    response = api_client.get(
        path=f'/workflows/{workflow.id}/events',
    )
...
def test_events__task_complete_fieldsets_absent__ok(api_client):
    ...
    """
    GET workflow events: TASK_COMPLETE row has task.fieldsets equal to null
    when the task has no FieldSet rows.
    """
...
def test_events__non_complete_task_fieldsets_null__ok(api_client):
    ...
    """
    GET workflow events: non-TASK_COMPLETE event still exposes task.fieldsets
    as null in serialized task payload.
    """
