...
from src.processes.enums import FieldRuleType, FieldSetRuleOperator
...
def test_retrieve__fieldset_all_data__ok(api_client):
    ...
    assert rule_resp['groups_or'][0]['api_name'] == group_or.api_name
    assert len(rule_resp['groups_or'][0]['groups_and']) == 1
    group_and_resp = rule_resp['groups_or'][0]['groups_and'][0]
...
def test_retrieve__soft_deleted_rulesets__excluded(api_client):

    """ Soft-deleted fieldset and field rulesets are not returned """
    ...
    # assert
    assert response.status_code == 200
    assert response.data['rulesets'] == []
    assert len(response.data['fields']) == 1
    assert response.data['fields'][0]['api_name'] == field.api_name
    assert response.data['fields'][0]['rulesets'] == []


def test_retrieve__soft_deleted_rule_group__excluded(api_client):

    """ Soft-deleted groups are not returned inside a live ruleset """
    ...
    FieldSetTemplateRuleGroupAnd.objects.filter(id=group_and.id).update(
        is_deleted=True,
    )
    api_client.token_authenticate(user=user)
    ...
    assert len(rule_resp['groups_or']) == 1
    assert rule_resp['groups_or'][0]['api_name'] == group_or.api_name
    assert rule_resp['groups_or'][0]['groups_and'] == []
