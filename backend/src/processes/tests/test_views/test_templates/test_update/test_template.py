...
def test_update__kickoff_with_one_fieldset__ok(
    ...
    assert response.status_code == 200
    kickoff = template.kickoff_instance
    link = FieldsetTemplateKickoff.objects.get(kickoff=kickoff)
    assert link.fieldset.api_name == fieldset.api_name
    assert link.order == 3


def test_update__kickoff_with_multiple_fieldsets__ok(
    mocker,
    api_client,
):
    ...
    assert len(links) == 2
    assert links[0].fieldset.api_name == 'fieldset-x'
    assert links[0].order == 0
    assert links[1].fieldset.api_name == 'fieldset-y'
    assert links[1].order == 1


def test_update__kickoff_with_empty_fieldsets__no_links_created(
    mocker,
    api_client,
):

    """ Updating a template with empty fieldsets list does not
...
def test_update__kickoff_without_fieldsets_key__no_links_created(
    mocker,
    api_client,
):
...
def test_update__task_with_one_fieldset__ok(
    ...
    # assert
    assert response.status_code == 200
    updated_task = TaskTemplate.objects.get(
        template=template,
        api_name=task.api_name,
    )
    link = FieldsetTemplateTaskTemplate.objects.get(task=updated_task)
    assert link.fieldset.api_name == fieldset.api_name
    assert link.order == 2


def test_update__task_with_multiple_fieldsets__ok(
    mocker,
    api_client,
):
...
def test_update__task_with_empty_fieldsets__no_links_created(
    mocker,
    api_client,
):
...
def test_update__task_without_fieldsets_key__no_links_created(
    mocker,
    api_client,
):

    """ Updating a template without fieldsets key in task does not
        create any FieldsetTemplateTaskTemplate records. """
