...
def test_create__only_required_fields__defaults_ok(api_client):
...
def test_create__all_fields__ok(api_client):
...
def test_create__kickoff_field_is_hidden__ok(api_client):
...
def test_create__fields_with_equal_api_names__save_last(api_client):
    # arrange
    user = create_test_user()
    api_client.token_authenticate(user)
...
def test_create__kickoff_field_with_dataset__ok(field_type, api_client):
...
def test_create__kickoff_field_dataset_null__ok(api_client):
...
def test_create__kickoff_field_selection_type_with_selections__ok(api_client):
