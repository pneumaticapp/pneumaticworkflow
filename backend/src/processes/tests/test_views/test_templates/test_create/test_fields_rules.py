...
def test_create_task_field_rules__show_date_owner_string_source__validation_error(
    ...
    Value is not a timestamp — 400 because owner is DATE.
    """
...
def test_create_task_field_rules__show_string_owner_date_source__ok(
    api_client,
):

    """
    SHOW on a STRING field that compares a DATE source.
    Value is checked against the owner type (STRING), not source.
    """
    ...
    group_and_data = (
        owner_data['rulesets'][0]['groups_or'][0]['groups_and'][0]
    )
    assert group_and_data['field'] == source_api_name
    assert group_and_data['value'] == '2020-01-01'
