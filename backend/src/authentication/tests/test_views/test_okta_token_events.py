...
def test_okta_token__existent_user__emit_user_login(
    ...
    save_tokens_mock = mocker.patch(
        'src.authentication.services.okta.'
        'OktaService.save_tokens_for_user',
    )
    ...
    get_user_profile_mock.assert_called_once_with('okta_access_token')
