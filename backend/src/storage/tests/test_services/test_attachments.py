...
class TestAttachmentServiceGetUserAttachments:

    def test_get_user_attachments__account__ok(self):
        # arrange
        user = create_test_admin()
        ...
        # act
        result = service.get_user_attachments(user)

        # assert
        assert attachment in result

    def test_get_user_attachments__public__ok(self):
        # arrange
        user = create_test_admin()
    ...
    def test_get_user_attachments__restricted_with_perm__ok(self):
        # arrange
        user = create_test_admin()
        workflow = create_test_workflow(user=user, tasks_count=1)
        task = workflow.tasks.first()
    ...
    def test_get_user_attachments__restricted_no_perm__not_in(self):
        # arrange
        user1 = create_test_admin()
        user2 = create_test_user(account=user1.account)
        ...
        assign_perm('storage.access_attachment', user1, attachment)

        service = AttachmentService(user=user2)

        # act
        result = service.get_user_attachments(user2)

        # assert
        assert attachment not in result

    def test_get_user_attachments__different_account__not_in(self):
        # arrange
        account1 = create_test_account()
        user1 = create_test_admin(account=account1)
        ...
        # act
        result = service.get_user_attachments(user1)

        # assert
        assert attachment not in result

    def test_get_user_attachments__distinct__ok(self):
        # arrange
        user = create_test_admin()
        ...
        assert result_ids.count(attachment.id) == 1
