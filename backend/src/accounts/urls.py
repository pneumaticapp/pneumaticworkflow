...
from src.accounts.views.api_key import APIKeyViewSet, UserAPIKeyViewSet
...
urlpatterns = [
    ...
    path(
        'users/<int:user_id>/api-keys',
        UserAPIKeyViewSet.as_view({'get': 'list', 'post': 'create'}),
    ),
    path(
        'users/<int:user_id>/api-keys/<int:pk>',
        UserAPIKeyViewSet.as_view({'delete': 'destroy'}),
    ),
